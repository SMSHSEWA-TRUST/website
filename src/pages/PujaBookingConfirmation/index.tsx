...
interface PujaBookingConfirmationPageProps {
    bookingData?: {
        pujaType: string;
...
const PujaBookingConfirmationPage: React.FC<PujaBookingConfirmationPageProps> = ({
    ...
    const bookingData = (location.state as any)?.bookingData || propBookingData;

    // If no booking data, redirect back to puja page
    if (!bookingData) {
        navigate('/puja');
        return null;
    }
    ...
    const handleGoBack = () => {
        // Redirect back to the main Pooja listing page
        navigate('/puja');
    };
    ...
    return (
        <div className="min-h-screen" style={{ backgroundImage: `linear-gradient(180deg, #8B0000 0%, #AD2F16 100%)` }}>
            {/* Header */}
            <div className="sticky top-0  px-6 py-4 flex justify-between items-center">
                <div className="flex items-center gap-4">
                    <button
                        onClick={handleGoBack}
                        className=" hover:bg-white/10 rounded-full transition-colors text-white"
                    >
                        <ArrowLeft className="w-6 h-6" />
                    </button>
                    <h1 className="text-xl font-bold text-white">Pooja Booking Confirmation</h1>
                </div>
                ...
                >
                    <div className="bg-white rounded-2xl shadow-xl w-full overflow-visible relative" style={{ borderRadius: 16, boxShadow: '0 8px 32px 0 rgba(0,0,0,0.15)' }}>
                        ...
                        <div className="relative w-full h-0">
                            <div className="absolute -bottom-2 left-0 w-5 h-5 bg-[#8B0000] rounded-full" style={{ zIndex: 2 }} />
                            <div className="absolute -bottom-2 right-0 w-5 h-5 bg-[#8B0000] rounded-full" style={{ zIndex: 2 }} />
                        </div>
                        {/* Dashed divider */}
...
export default PujaBookingConfirmationPage;